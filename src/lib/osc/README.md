...
| `../clock/ClockSync.ts`  | The clock's main-thread half (composed by OscClient): the min-RTT sample window, `now()`, the store publish, the TICK-driven callback registry (the metronome is the audio engine's `/tr`, id 4242 at 20 Hz), and the composed one-way `TickTracker` (`audioNow()`/skew from the phase payload). The estimate survives a worker respawn.                                                   |
...
`/tr` tick (the `__global_clock__` synth, trigger id 4242 at 20 Hz —
AUDIO-CLOCK.md), routed by trigger id in `handleReply` so a plugin's own
SendTrig passes untouched. Clock callbacks fire only while connected and
ticking (nothing keeps time offline; see docs/clock.md). `clockNow()` is
wall time plus the latest estimated offset.
