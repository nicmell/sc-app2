...
): OSC.Message => new OSC.Message('/b_set', bufnum, ...pairs.flat());
...
export const bGetn = (
  ...
  start: number,
  count: number,
): OSC.Message => new OSC.Message('/b_getn', bufnum, start, count);
...
export const bSetSampleRate = (
  bufnum: number,
  sampleRate: number,
): OSC.Message => new OSC.Message('/b_setSampleRate', bufnum, sampleRate);
