...
| `adsr-panel-plugin`     | CLIENT-side envelope construction: four knob-bound vars feed a DERIVED var through `pad(adsr(att, dec, sus, rel), 36)` — every knob move recomputes the run, every NEW keyboard voice latches the current shape. Tests function calls with state args in `bind:value`.  |
| `mod-env-plugin`        | SERVER-side modulatable envelope: `perc(att, rel)` on EnvGen's variadic `envelope` input wires the def PARAMS into the envelope's time slots — knobs `/n_set` the PLAYING node and scsynth recomputes the shape (watch it morph on the unipolar scope). Tests call lowering with modulatable refs.  |
