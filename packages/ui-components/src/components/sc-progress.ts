...
export class ScProgressBase extends LitElement {
  ...
  /** Light DOM so the global `.sc-progress*` classes apply. */
  ...
  render() {
    ...
    const cls = classMap({
      "sc-progress": true,
      [`sc-progress--${this.variant}`]: true,
      [`sc-progress--${this.size}`]: true,
      "is-indeterminate": indeterminate,
      "is-determinate": !indeterminate,
    });
    ...
    return html`<div
      ...
      aria-busy=${busy}
    >
      <div class="sc-progress__fill" style=${fillStyle}></div>
    </div>`;
  }
}
