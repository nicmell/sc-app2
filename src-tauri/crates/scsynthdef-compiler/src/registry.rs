...
/// committed spec at `assets/specs/ugens.json` (repo root). All payloads
/// are `'static` references, so entries are `const`-constructible and
/// live directly in the binary.
///
/// Emitted by `build.rs` into `OUT_DIR` and included by the private
/// `specs` sibling module — edit the spec, never the expansion.
...
pub struct UGenRegistryEntry {
    ...
    /// Per-argument documentation, in argument declaration order.
