...
SessionManager (src/lib/sessions)      orchestrates a session; depends only on ↓
   ...
   OscClient (OscClient.ts)            the seam — ready/sendCommand/onReply/onError/onScopeChunk/dispose
        ...
        ├── WorkerOscClient            real Worker — browser AND node worker_threads
        └── InProcessOscClient (test)  no worker; wraps the bridge directly
...
(The Node clients/entry — `InProcessOscClient`, `createNodeWorkerClient`,
`nodeWorkerEntry` — live in `test/clients/`, since they're test-only.)
...
- **`bridge` takes callbacks, not the worker protocol** — so it can be driven *with* a
  worker (`runOscWorker`) *or* directly in-process (`InProcessOscClient`, the fast
  hermetic test client). This is the one decision that makes both test tiers possible.
- **`transport` is OSC-free** — a reusable, independently testable WebSocket pipe.

## Tests

`test/client.test.ts` runs the suite over **both** `InProcessOscClient` and the
`worker_threads` client against an in-process mock bridge. `test/scenarios/*`
are opt-in `tsx` runs against the real Rust `serve` + a fake scsynth.
