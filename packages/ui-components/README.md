...
| `sc-base-text`        | `as` `size` `weight` `tone` `font` `align` `transform` `truncate` `inline`            | —                | typography; `as` = `span`/`p`/`div`/`label`/`h1`–`h6`; `transform`=`uppercase` adds eyebrow tracking (caps-label) |
...
| `sc-base-flex`        | `orientation` `wrap` `justify` `align` `gap`                                          | —                | wrapper-free flex layout; horizontal/vertical, wrapping, axis alignment, tokenized gaps                           |
| `sc-base-row`         | `align` `justify` `gutter` `wrap`                                                     | —                | 24-unit grid row; tokenized gutters and flex-axis alignment                                                       |
| `sc-base-col`         | `span` `offset` `order` `push` `pull` `flex`                                          | —                | grid column; fixed 1–24 spans or free-form flex sizing                                                            |
...
`bar`(d)/`spinner`; badge/chip/alert/toast/`sc-text`-`tone` carry state palettes. Flex
`gap` is `none`(d)/`xs`/`sm`/`md`/`lg` → `0`/`--space-*`.
