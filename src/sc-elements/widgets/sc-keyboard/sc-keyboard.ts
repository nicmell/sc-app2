...
import { baseRuntime, failValidation, requireNoScChildren, resolveNode } from "@/sc-elements/internal/validation";
...
export class ScKeyboard extends ScElement {
   ...
   *  <sc-synthdef> in scope. A leaf otherwise — no children, no node. */
  ...
  async noteOn(note: number, velocity: number): Promise<void> {
    ...
    try {
      const nodeId = await oscClient.createSynth(this._synthdef, groupId, {
