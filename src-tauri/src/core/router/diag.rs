...
/// (`numChildren == -1`, def name + optional controls) or a group recursing
/// into its inline depth-first children. Malformed args degrade per-field
/// (MIN id, empty def, Null) rather than failing the tree.
...
/// Top level: `[flag, queriedGroupId, childCount, nodes…]` →
