...
| `keyboard-plugin`       | A `sc-keyboard` over a self-freeing `sineNote` synthdef (`EnvGen` + `<sc-env shape="adsr">`, `action="2"`): each pressed key spawns a voice (`freq` = note.midicps, `amp` = velocity), key-up sets `gate` → 0 and the envelope releases + frees the node. Tests the widget's transient-voice spawn/release into the plugin group and the `<sc-env>` envelope input. Play by click, the `a…k` row, or MIDI hardware. |
