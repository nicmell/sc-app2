// The main-thread proxy to the WebSocket worker: the same `OscTransport`
...
// go over it via open/close commands. Status is tracked here from the
// transport's events — it can't be queried synchronously across the thread
// boundary.

import { TRANSPORT_STATUS, type OscTransport } from "./transport";
import type { TransportCommand, TransportEvent } from "@/types/osc";

export class WorkerClient implements OscTransport {
  ...
  private socketStatus: number = TRANSPORT_STATUS.IS_NOT_INITIALIZED;
  private notify: (msg: TransportEvent) => void = () => {};

  constructor() {
    this.worker.onmessage = (ev: MessageEvent<TransportEvent>) => {
      const msg = ev.data;
    ...
    this.worker.onerror = (ev: ErrorEvent) => {
      this.notify({ type: "error", message: ev.message });
    };
  }
  ...
  close(): void {
    if (this.socketStatus === TRANSPORT_STATUS.IS_NOT_INITIALIZED || this.socketStatus === TRANSPORT_STATUS.IS_CLOSED) {
      return;
    }
