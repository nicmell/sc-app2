...
includes the committed `src/builders_wasm_gen.rs`, which contains the
...
needed. It emits committed `src/specs/` and `src/builders/` modules per
category, plus `src/builders_wasm_gen.rs` and `src/env_shapes.rs`, and passes
