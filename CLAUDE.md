...
lib/                     non-React infrastructure
  ...
  osc/                   the worker-resident OSC stack (see lib/osc/README.md):
                         ...
                         composition root + test seam; worker/transport owns the
                         raw WebSocket and worker/worker is the bundled entry
...
2. Connection: SessionLayout's effect hands the loader's SessionInfo to
   ...
   of scsynth's root group 0**; synth ids are allocated inside the OSC worker
...
internally), built on worker-side FIFO reply waiters (matched in
...
6. **Input elements** — DONE. Value dispatch over the shared state seam (see
   ...
   worker-side `send` with an auto-responder through `handleReply`; interaction
