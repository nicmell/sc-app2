...
vocabulary. The METRONOME is the audio engine's own `/clock/tick` (the
`__global_clock__` synth's SendReply, 20 Hz, carrying the absolute tick
index — AUDIO-CLOCK.md), routed by address in `handleReply`; `/tr`
belongs entirely to the plugins. Clock callbacks fire only while connected and ticking (nothing
